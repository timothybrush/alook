...
test.describe.serial("auth & first screen", () => {
  test("unauthenticated /community deep-link redirects to /sign-in with redirect param", async ({ browser }) => {
    const context = await browser.newContext() // no storageState — anonymous
    const page = await context.newPage()
    await page.goto("/community/channels/does-not-matter/whatever")
